...
目前还导出 `getTabList`、`getPageRef`、`setTags`、`getTags`、`setTagColor` 与 `openIIBInNewTab`。类型定义见 [useGlobalStore.ts](../vue/src/store/useGlobalStore.ts)，实现见 [defineExportFunc.ts](../vue/src/defineExportFunc.ts)。旧版标签搜索、模糊搜索及独立结果页已移除，不再通过 `insertTabPane` 支持这些视图类型。
