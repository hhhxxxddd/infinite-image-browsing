...
- **按需接入 AI**：下载或选用已有的 Qwen3-VL 检索、重排与内容处理模型，生成描述、参考提示词和标签建议；内容处理还可选择 OpenRouter。模型权重不随源码或安装包分发。

媒体索引与用户补充的信息保存在本地数据库；日常整理不会搬动原文件。复制、移动、重命名、删除和图片编辑等明确的文件操作会修改磁盘内容或生成副本。

本地索引与本地模型推理不需要把媒体上传到云端。只有选择 OpenRouter 内容处理时，待处理图片会发送给所配置的服务；具体行为见 [AI 接入](docs/qwen3-vl-search.md)。
