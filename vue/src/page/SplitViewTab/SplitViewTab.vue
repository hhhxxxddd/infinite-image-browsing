...
import { AppstoreOutlined, PictureOutlined, VideoCameraOutlined, FolderOutlined, ApartmentOutlined, SettingOutlined, PlusOutlined, HistoryOutlined, CloseOutlined, CompassOutlined, SplitCellsOutlined } from '@ant-design/icons-vue'
  ...
  <div class="media-app" :class="{ compact }">
    <aside class="app-sidebar" aria-label="主导航">
      ...
      <nav class="nav-scroll">
        ...
        <div class="nav-caption">标签页</div><p v-if="!openViews.length" class="sidebar-hint">点击目录节点，在这里打开</p><div v-for="entry in openViews" :key="entry.pane.key" class="open-view"><button class="nav-item" :class="{ selected: current?.pane.key === entry.pane.key }" :data-drop-active="entry.pane.type === 'local' && dropTarget === entry.pane.path" :title="entry.pane.type === 'local' ? `拖动文件到：${entry.pane.path}` : undefined" @dragover="entry.pane.type === 'local' && entry.pane.path && folderDragOver($event, entry.pane.path)" @dragleave="dropTarget = ''" @drop="entry.pane.type === 'local' && entry.pane.path && dropIntoFolder($event, entry.pane.path, entry.pane.key)" @click="focus(entry.pane.key)"><FolderOutlined v-if="entry.pane.type === 'local'" /><HistoryOutlined v-else /><span>{{ entry.pane.nameFallbackStr || (typeof entry.pane.name === 'string' ? entry.pane.name : pageNames[entry.pane.type]) }}</span></button><button class="close-view" :aria-label="`关闭标签页：${entry.pane.nameFallbackStr || (typeof entry.pane.name === 'string' ? entry.pane.name : pageNames[entry.pane.type])}`" @click="close(entry.tabIdx, entry.pane.key)"><CloseOutlined /></button></div>
      </nav>
    ...
    <main class="app-main">
      <header class="app-header" :class="{ 'library-header': hasLibrarySearch }">
        ...
        <div class="header-actions">
          ...
          <a-button class="header-action-icon" :type="imageComparison.drawerVisible ? 'primary' : 'text'" title="图片对比与多图查看" aria-label="图片对比与多图查看" @click="imageComparison.openComparison()"><SplitCellsOutlined /></a-button>
